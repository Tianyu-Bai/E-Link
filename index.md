...
.logo-mask-container::after {
  ...
  background: linear-gradient(
    to right, 
    transparent 0%, 
    rgba(96, 165, 250, 0.4) 10%,   /* 柔光区极窄，迅速亮起 */
    rgba(167, 139, 250, 0.95) 25%, /* ✨ 高光区提早开始 ✨ */
    rgba(167, 139, 250, 0.95) 75%, /* ✨ 高光区极度宽广（占据一半以上的体积） ✨ */
    rgba(96, 165, 250, 0.4) 90%,   /* 柔光区缓慢暗下 */
    transparent 100%
  );
  ...
  /* 总时长改为 5s，足够慢，且节奏紧凑 */
  animation: searchlight-sweep 5s linear infinite;
}

@keyframes searchlight-sweep {
  /* 0%：光束右侧边缘刚刚碰到 Logo 左侧，没有任何空跑 */
  0% { transform: translateX(-100%) skewX(-20deg); }
  
  /* 85%：花足足 4.25 秒的时间（5秒的85%），让这团巨大的光晕非常缓慢地滑过 Logo */
  85% { transform: translateX(100%) skewX(-20deg); }
  
  /* 85% - 100%：只有 0.75 秒的干净停顿（用于换气），然后立刻开始下一次慢扫 */
