...
// 1. 保留原始颜色序列
const intanColors = [
'#e04a4a', '#d49b38', '#522e8a', '#3dc94d', // 把原本第3个灰色改成了Intan标志性的紫色
...
function generateChannels(prefix) {
  ...
  function resizeIntanCanvas() {
    ...
    // 🚀 核心修复：确保重置 scanX 时，能适应手机端的窄边框 (75)
    // 如果没有这一行，手机端加载时 scanX 默认为 95，会画在彩色框后面导致波形“消失”
...
function drawPane(ctx, channelsData, isLeftPane) {
    ...
    // 💡 模拟宏观皮层状态 (Global Cortical State)：混合 1.5Hz(Delta) 和 4Hz(Theta) 的全局慢波
    ...
    for (let i = 0; i < NUM_CHANNELS; i++) {
        ...
        if (ch.isBad) {
            // 【物理特征：Notch 60Hz 开启状态下的坏通道】
            // 60Hz 正弦波被滤除，暴露出悬空电极极不稳定的极低频漂移 (1/f) 和放大器热噪声
